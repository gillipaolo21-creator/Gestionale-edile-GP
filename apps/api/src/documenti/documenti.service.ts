...
export class DocumentiService implements OnModuleInit {
  ...
  async updateStato(documentoId: string, stato: string): Promise<Documento> {
    ...
    const currentJson = (doc.datiEstrattiJson as Record<string, unknown>) ?? {};
    return this.prisma.documento.update({
      where: { id: documentoId },
      data: { datiEstrattiJson: { ...currentJson, stato } as Prisma.InputJsonValue },
    });
  }
  ...
  async exportCommessaZip(commessaId: string): Promise<{ stream: NodeJS.ReadableStream; filename: string }> {
    ...
    const docs = await this.prisma.documento.findMany({
      where: { entitaTipo: TipoEntitaDocumento.COMMESSA, entitaId: commessaId },
    });

    // Use JSZip (bundled with xlsx) for ZIP creation
