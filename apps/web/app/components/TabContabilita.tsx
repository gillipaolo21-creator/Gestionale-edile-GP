  ...
  if (loading) {
    ...
    salContent = (
      <div className="relative">
        ...
        <div className="space-y-4">
          {sals.map((sal, idx) => {
            const meta = SAL_STATE_LABELS[sal.stato] ?? { label: sal.stato, color: 'bg-stone-100 text-stone-600' };
            return (
                ...
                <div className="flex-1 bg-white border border-stone-100 rounded-2xl p-5 shadow-sm">
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <p className="text-lg font-black text-[#003A7D]">€ {Number.parseFloat(sal.importoMaturato).toLocaleString('it-IT', { minimumFractionDigits: 2 })}</p>
                    </div>
                  ...
                  <div className="w-full bg-stone-100 rounded-full h-1.5 mt-3">
                    <div
                      className="bg-[#0054B4] h-1.5 rounded-full transition-all duration-700"
                      style={{ width: `${Math.min(100, Number.parseFloat(sal.percentualeCompletamento))}%` }}
                    />
                  </div>
                  <p className="text-[9px] text-stone-400 mt-1 font-semibold">{Number.parseFloat(sal.percentualeCompletamento).toFixed(1)}% completamento</p>
                </div>
              </div>
            );
          })}
    ...
    fattureContent = (
      <div className="space-y-2">
        {fatture.map(f => {
          ...
          return (
            <div key={f.id} className="flex items-center justify-between bg-white border border-stone-100 rounded-2xl px-6 py-4 shadow-sm animate-in fade-in duration-300">
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[9px] text-stone-400 uppercase tracking-widest">Scadenza: {f.dataScadenza ? new Date(f.dataScadenza).toLocaleDateString('it-IT') : '—'}</p>
                </div>
              </div>
    ...
    <div className="animate-in fade-in slide-in-from-bottom-2 space-y-8">
      ...
      {activeSection === 'sal' && (
        <div className="space-y-4">
          ...
          {showSalForm && (
            <div className="bg-white border border-stone-200 rounded-2xl p-6 space-y-4 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
              <p className="text-[9px] font-black text-[#003A7D] uppercase tracking-widest">Nuovo SAL</p>
              <div className="grid grid-cols-3 gap-4">
                <div>
                  <label htmlFor="sal-data-cert" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">Data Certificazione</label>
                ...
                <div>
                  <label htmlFor="sal-perc" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">% Completamento</label>
                ...
                <div>
                  <label htmlFor="sal-importo" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">Importo Maturato (€)</label>
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={() => setShowSalForm(false)} disabled={saving} className="px-4 py-2 text-[9px] font-black uppercase tracking-widest text-stone-400 hover:text-stone-600 transition-colors disabled:opacity-50">Annulla</button>
                <button onClick={handleAddSal} disabled={saving} className="px-5 py-2 bg-[#003A7D] text-white text-[9px] font-black uppercase tracking-widest rounded-lg hover:bg-[#0054B4] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
          ...
          {showFatturaForm && (
            <div className="bg-white border border-stone-200 rounded-2xl p-6 space-y-4 shadow-sm animate-in fade-in slide-in-from-top-2 duration-300">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="fat-tipo" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">Tipo</label>
                ...
                <div>
                  <label htmlFor="fat-sal" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">SAL Collegato</label>
                ...
                <div>
                  <label htmlFor="fat-importo" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">Imponibile (€)</label>
                ...
                <div>
                  <label htmlFor="fat-iva" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">IVA (%)</label>
                  <input id="fat-iva" type="number" value={fatturaForm.iva} onChange={e => setFatturaForm(f => ({ ...f, iva: e.target.value }))}
                    className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm text-[#003A7D] focus:outline-none focus:border-[#0054B4]" />
                </div>
                <div>
                  <label htmlFor="fat-scadenza" className="text-[9px] font-black uppercase tracking-widest text-stone-400 block mb-1">Data Scadenza</label>
                  <input id="fat-scadenza" type="date" value={fatturaForm.dataScadenza} onChange={e => setFatturaForm(f => ({ ...f, dataScadenza: e.target.value }))}
                    className="w-full border border-stone-200 rounded-lg px-3 py-2 text-sm text-[#003A7D] focus:outline-none focus:border-[#0054B4]" />
                </div>
              </div>
              <div className="flex gap-2 justify-end">
                <button onClick={() => setShowFatturaForm(false)} disabled={saving} className="px-4 py-2 text-[9px] font-black uppercase tracking-widest text-stone-400 hover:text-stone-600 transition-colors disabled:opacity-50">Annulla</button>
                <button onClick={handleAddFattura} disabled={saving} className="px-5 py-2 bg-[#003A7D] text-white text-[9px] font-black uppercase tracking-widest rounded-lg hover:bg-[#0054B4] transition-colors disabled:opacity-60 disabled:cursor-not-allowed">
