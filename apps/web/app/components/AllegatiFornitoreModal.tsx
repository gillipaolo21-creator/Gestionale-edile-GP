...
interface AllegatiFornitoreModalProps {
  isOpen: boolean;
  fornitori: string[];
  onClose: () => void;
  onSubmit: (files: FileList, ragioneSociale: string, descrizione: string) => void;
}

export function AllegatiFornitoreModal({ isOpen, fornitori, onClose, onSubmit }: AllegatiFornitoreModalProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedFornitore || files.length === 0) return;
    const dt = new DataTransfer();
    files.forEach(f => dt.items.add(f));
    onSubmit(dt.files, selectedFornitore, descrizione);
    handleClose();
  };
