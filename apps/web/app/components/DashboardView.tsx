'use client';
import { AlertCircle, Briefcase, ChevronRight, FileText, MapPin, Plus, Trash2 } from 'lucide-react';
...
export function DashboardView({ commesse, stats, pendingDocs, onOpenDetail, onPreviewDoc, onCreateCommessa, onDeleteFromHome }: DashboardViewProps) {
  return (
    <>
      ...
      <div className="grid grid-cols-3 gap-8 mb-16">
        {[
          ...
          <div key={i} className="bg-white p-6 rounded-2xl border border-stone-400 shadow-xl shadow-stone-400/50">
                    ...
                    <div className="flex items-center gap-6">
                      ...
                      <div className="flex flex-wrap items-center gap-3 text-lg text-[#003A7D] tracking-tight">
                        ...
                        <span className="font-semibold text-stone-600 flex items-center gap-1">
                          <MapPin size={12} />
                          {c.indirizzo ? `${c.indirizzo}${c.citta ? ` (${abbreviaCitta(c.citta)})` : ''}` : (c.citta ? abbreviaCitta(c.citta) : 'N/D')}
                        </span>
                      </div>
                    </div>
        ...
        {pendingDocs.length > 0 && (
          <section className="space-y-4">
            ...
            <div className="bg-white rounded-2xl border border-stone-400 shadow-xl shadow-stone-400/50 overflow-hidden divide-y divide-stone-100 sticky top-8">
              {pendingDocs.map(doc => {
                const meta = doc.datiEstrattiJson as Record<string, unknown> | null;
